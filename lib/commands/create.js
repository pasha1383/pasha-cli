...
const { renderTemplateDir, renderModuleFiles } = require('../core/engine');
...
async function create() {
  ...
  // Ask how many modules to scaffold, then collect each name individually.
  let modules = [];
  if (templateConfig.modules?.enabled) {
    log.title('🧱 Modules');
  ...
  const ctx = { ...baseAnswers, ...extraAnswers, modules };
  ...
  try {
    ...
    // Shared files render once; module-scoped files render once per module.
    ...
    if (modules.length && (await fs.pathExists(moduleFilesDir))) {
      await renderModuleFiles(moduleFilesDir, outDir, ctx, modules);
    }
  ...
  if (templateConfig.postInstall?.includes('npm install')) {
    ...
    // Resolve npm's absolute path ourselves rather than relying on PATH lookup
    // at spawn time — guarantees we run the exact binary the prerequisite check
    // already confirmed exists, and lets us report clearly if it can't be found.
    ...
    if (!npmPath) {
      ...
      try {
        // stdio: 'inherit' streams npm's real output live — if it fails,
        // you see npm's own error messages directly, not a generic warning.
