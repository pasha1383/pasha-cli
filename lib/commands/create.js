...
async function create() {
  ...
  if (templateConfig.postInstall?.includes('npm install')) {
    ...
    // Resolve npm's absolute path ourselves instead of letting execa search
    // PATH again — guarantees we run the exact binary the prerequisite check
    ...
    if (!npmPath) {
      ...
      try {
        ...
        await execa(npmPath, ['install'], { cwd: outDir, stdio: 'inherit' });
        log.ok('npm install done');
      } catch (err) {
        log.fail(`npm install failed (code: ${err.code ?? 'n/a'}, exit code: ${err.exitCode ?? 'n/a'})`);
        if (err.shortMessage) console.log(chalk.red(err.shortMessage));
        log.warn(`Reproduce manually: cd ${ctx.projectName} && ${npmPath} install`);
      }
    }
  }
