...
const featuresGoStdlib = require('../core/features-go-stdlib');
...
function resolveFeatures(stackFlavor) {
  const frontendFlavors = ['nextjs', 'react', 'vue', 'svelte', 'angular', 'astro', 'html'];
  if (frontendFlavors.includes(stackFlavor)) {
    const frontendShared = require('../core/features-frontend-shared');
    return frontendShared;
  }
  switch (stackFlavor) {
    case 'express': return featuresExpress;
    case 'fastify': return featuresFastify;
    case 'go':        return featuresGo;
    case 'go-stdlib': return featuresGoStdlib;
...
async function askCoreStack(fm, ctx) {
  ...
  if (!isFrontend) {
    ...
    const redisAnswer = await inquirer.prompt([{
      type: 'confirm', name: 'useRedis', message: 'Add Redis for caching?', default: false,
    }]);
    if (redisAnswer.useRedis === '__back__') return '__back__';
    useRedis = redisAnswer.useRedis;

    const brokerAnswer = await inquirer.prompt([{
      type: 'list', name: 'broker', message: 'Message broker?',
      choices: withBack(fm.brokerChoices()),
    }]);
    if (brokerAnswer.broker === '__back__') return '__back__';
    broker = brokerAnswer.broker;
  }
...
function buildContext(flags, modules, baseAnswers, ctx) {
  ...
  } else if (flavor === 'go' || flavor === 'go-stdlib') {
    const deps = fm.resolveDependencies(flags);
...
async function runMicroservices(ctx) {
  ...
  for (let i = 0; i < Number(count); i++) {
    ...
    tui.serviceSummary(finalCtx, i + 1);
  }
