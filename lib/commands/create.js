...
/**
 * Walks the questions common to every framework: ORM, database (filtered by
 * ORM), validation, Redis, broker, AGENT.md. Each answer narrows the next
 * question, so the user is never offered a combination that cannot actually
 * work (Mongoose with PostgreSQL, a database question when they picked no
 * ORM, and so on). Takes the features module so the same flow serves both
 * NestJS and Express — they share the exact same ORM/database/broker
 * choices, only the validation options differ (class-validator vs
 * express-validator).
 */
...
async function askStackFeaturesExpress() {
  return askCoreStack(featuresExpress);
}
...
function printNextSteps(ctx) {
  ...
  if (ctx.useSwagger) {
    console.log(chalk.gray('  # then open http://localhost:3000/api/docs'));
  }
  console.log('');
}
...
async function create() {
  ...
  // "nestjs" and "express" pick which questions get asked and which
  // dependency/flag resolution runs — everything downstream (rendering,
  // fileConditions) just reads flags off ctx and doesn't care which one it was.
  const stackFlavor = templateConfig.stackFeatures; // false | 'nestjs' | 'express'
  const featuresModule = stackFlavor === 'express' ? featuresExpress : features;
  const askStack = stackFlavor === 'express' ? askStackFeaturesExpress : askStackFeaturesNest;
