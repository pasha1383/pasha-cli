...
function resolveDependencies(flags) {
  ...
  if (flags.useLint) {
    ...
    devDependencies['eslint-plugin-svelte'] = '^3.0.0';
    ...
    devDependencies['prettier-plugin-svelte'] = '^3.3.0';
  }

  if (flags.useTests) {
    devDependencies.vitest = '^3.0.0';
    devDependencies['@testing-library/svelte'] = '^5.0.0';
...
function resolveScripts(flags) {
  ...
  if (flags.useLint) {
    scripts.lint = 'eslint "src/**/*.{svelte,ts,js}" --fix';
    scripts.format = 'prettier --write "src/**/*.{svelte,ts,js,css}"';
  }
